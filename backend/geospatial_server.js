...
async function callLocalLLM(systemPrompt, userPrompt, temperature = 0.2) {
  try {
    ...
    // Extract the text content from the Chat API structure
    const content = data.message?.content || "";
    
    console.log(` LLM responded with ${content.length} characters`);
    return content;
...
Return ONLY valid JSON following the specified IR schema. Do NOT guess variable names.
...
Be thorough but precise. Extract all relevant analytical steps from the user's query. Do not include any other text than the JSON.`;
